...
fn main() {
    ...
    let idl_path = out_dir_path.join("factory_vara_dex.idl");
    let client_path = out_dir_path.join("factory_vara_dex_client.rs");

    // Generate the IDL file
    // sails_idl_gen::generate_idl_to_file::<contract::ContractProgram>(&idl_path)
    sails_idl_gen::generate_idl_to_file::<factory_vara_dex::FactoryVaraDexProgram>(&idl_path)
        .unwrap();
